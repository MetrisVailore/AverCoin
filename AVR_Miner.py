#!/usr/bin/env python3
"""
Aver-Coin Official AVR Miner 1.0
https://avercoin.com
https://github.com/AverCoin/avercoin
Aver-Coin Team & Community 2024-2025
"""
...
UDP_MAX_SIZE = 65535
server_address = "80.66.64.188"
server_port = 5000

# Python <3.5 check
f"Your Python version is too old. Aver-Coin Miner requires version 3.6 or above. Update your packages and try again"
...
class Settings:
    ...
    AVR_TIMEOUT = 10
    BAUDRATE = 115200
    DATA_DIR = "Aver-Coin AVR Miner " + str(VER)
    SEPARATOR = ","
    ENCODING = "ascii"
    TEMP_FOLDER = "Temp"
    ...
    try:
        ...
        if float(Settings.VER) < float(data["tag_name"]):  # If is outdated
            ...
            if update == "Y" or update == "y":
                ...
                DATA_DIR = "Aver-Coin AVR Miner " + str(data["tag_name"])  # Create new version config folder
                ...
                download_url = "https://github.com/revoxhere/duino-coin/releases/download/" + data[
                    "tag_name"] + "/" + zip_file
...
class Client:
    ...
    def connect(pool: tuple):
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # s = socket()
    ...
    def sendmessage(s, msg: str):
        s.send(str(msg).encode('ascii'))  # .encode(Settings.ENCODING))
        return True

    def recv(s, limit: int = UDP_MAX_SIZE):  # 128
        data = s.recv(UDP_MAX_SIZE).decode("ascii")  # .decode(Settings.ENCODING).rstrip("\n")
        return data
        ...
        while True:
            ...
            try:
                ...
                if response["success"] == True:
                    ...
                    NODE_ADDRESS = server_address  # NODE_ADDRESS = response["ip"]
                    NODE_PORT = server_port  # NODE_PORT = response["port"]
...
def load_config():
    ...
    if not Path(str(Settings.DATA_DIR) + '/Settings.cfg').is_file():
        ...
        while not correct_username:
            ...
            if not username:
                username = choice(["revox", "Metris"])

            print("Публичный адрес должен начинатся с pbc")
            if username.startswith("pbc"):
                correct_username = True
            else:
                correct_username = False
            if not correct_username:
                username = "pbc3aa31bd41d19563394154345823fcdcf6a6f72e78bef99fc8058fa9ea5fae336"
    ...
    while True:
        try:
            ...
            RPC.update(details="Hashrate: " + str(total_hashrate),
                       ...
                       large_text="Aver-Coin, "
                                  + "a coin that can be mined with almost everything"
                                  + ", including AVR boards",
                       buttons=[{"label": "Visit avercoin.com (еще не выпущено)",
                                 "url": "https://avercoin.com"},
                                {"label": "Join the Telegram Channel / Присоединитесь к телеграм каналу",
                                 "url": "https://t.me/avercoin"}])
        except Exception as e:
    ...
    while True:
        ...
        while True:
            try:
                ...
                server_version = Client.recv(s, UDP_MAX_SIZE)
                ...
                if threadid == 0:
                    ...
                    Client.sendmessage(s, "MOTD")
                    motd = Client.recv(s, UDP_MAX_SIZE)
        ...
        while retries < 3:
            ...
            pretty_print('sys' + port_num(com),
                         ...
                         + "and your device is supported by Aver-Coin.",
                         'error')
            break
        ...
        while True:
            try:
                ...
                job = Client.recv(s, UDP_MAX_SIZE).split()
            ...
            try:
                Client.sendmessage(s, str(num_res)
                                   + ' '
                                   + str(hashrate_t)
                                   + ' '
                ...
                feedback = Client.recv(s, UDP_MAX_SIZE).split(",")
...
if __name__ == '__main__':
    ...
    if discord_presence == "y":
        try:
            ...
            debug_output(f'Error launching Discord RPC thread: {e}')  # !/usr/bin/env python3
